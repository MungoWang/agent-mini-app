 ...
 *
 * 1. author specifiers — UI: react + @monkey-mini-app/ui + lodash; backend: @monkey-mini-app/api + lodash; + relative paths
...
import { readThemeFileKeys, REQUIRED_THEME_FILE_KEYS } from "../gen/skill/theme.mjs"
...
// plus the platform vendor `lodash` / `lodash-es`.
...
/* ------------------------------------------------ 4d. generated theme.md drift */

// references/theme.md is generated from globals.css, so a token in the doc that is not in
// the stylesheet means the file was hand-edited or gen:skill was skipped. An agent trusts
// this table to pick colour tokens — a stale entry silently renders as `unset`.
{
 const themeMd = path.join(skillDir, "references/theme.md")
 const globalsCss = path.join(root, "packages/ui/src/styles/globals.css")
 if (!fs.existsSync(themeMd)) {
  fail("theme-doc", skillDir, "references/theme.md is missing — run pnpm gen:skill")
 } else if (!fs.existsSync(globalsCss)) {
  fail("theme-doc", globalsCss, "stylesheet missing — update check/skill.mjs")
 } else {
  const declared = new Set(
   [...fs.readFileSync(globalsCss, "utf8").matchAll(/(--[a-z0-9-]+)\s*:/g)].map((m) => m[1])
  )
  const doc = fs.readFileSync(themeMd, "utf8")
  if (!/GENERATED by scripts\/gen\/skill\/theme\.mjs/.test(doc)) {
   fail("theme-doc", themeMd, "missing the GENERATED header — it must come from gen:skill, not by hand")
  }
  const splitAt = doc.indexOf("## Custom host theme file")
  if (splitAt < 0) {
   fail("theme-doc", themeMd, "missing ## Custom host theme file — rerun pnpm gen:skill")
  }
  const appPart = splitAt < 0 ? doc : doc.slice(0, splitAt)
  const customPart = splitAt < 0 ? "" : doc.slice(splitAt)
  const cited = new Set([...appPart.matchAll(/`(--[a-z0-9-]+)`/g)].map((m) => m[1]))
  for (const token of [...cited].sort()) {
   if (!declared.has(token)) {
    fail("theme-doc", themeMd, `${token} is documented but not defined in globals.css — rerun pnpm gen:skill`)
   }
  }
  // A core token that vanished from the doc means the generator stopped seeing it.
  for (const must of ["--background", "--foreground", "--primary", "--muted", "--border", "--card"]) {
   if (declared.has(must) && !cited.has(must)) {
    fail("theme-doc", themeMd, `${must} exists in globals.css but the generated table dropped it`)
   }
  }
  try {
   const fileKeys = readThemeFileKeys()
   const fileCited = new Set([...customPart.matchAll(/`--([a-z0-9-]+)`/g)].map((m) => m[1]))
   for (const key of fileKeys) {
    if (!fileCited.has(key)) {
     fail("theme-doc", themeMd, `custom theme file key --${key} missing — rerun pnpm gen:skill`)
    }
   }
   for (const key of REQUIRED_THEME_FILE_KEYS) {
    if (!fileKeys.includes(key)) {
     fail("theme-doc", path.join(root, "packages/panel/src/themes.ts"), `required file key ${key} not in THEME_VAR_KEYS`)
    }
   }
  } catch (cause) {
   fail("theme-doc", themeMd, cause instanceof Error ? cause.message : String(cause))
  }
 }
