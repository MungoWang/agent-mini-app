...
# Inputs: this repo; env DSH_HOME
# Writes: ~/.dsh/profiles/web/** and its runtime/host.json
# Side effects: machine — may install dsh/pnpm, path-links repo packages, runs pnpm add
# Run as: manual — a developer machine only; users run `dsh plugin add @monkey-mini-app/dsh-mini-app`
set -euo pipefail
...
(
  ...
  if [ -x "$ROOT/node_modules/.bin/tsx" ]; then
    "$ROOT/node_modules/.bin/tsx" "$ROOT/scripts/setup/host-config.mts"
  else
    pnpm exec tsx "$ROOT/scripts/setup/host-config.mts"
  fi
)
