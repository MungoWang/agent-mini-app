# 任务归档：per-app backend packages（`mini_app_install`）

> 归档日期：2026-09-05 · 原位置：`TODO.md` → NEXT 块
> 设计：[`docs/rfcs/per-app-packages.md`](../../rfcs/per-app-packages.md) §1–§6（§7 审批弹窗仍未实现，等分享包）
> 落地：`packages/host/src/apps/app-packages.ts` + `mini_app_install` + 第 8 个模板 `spreadsheet`

## 实现时与计划不一致的地方（有价值的偏差）

- **模板不预置 `package.json`**：清单是 `mini_app_install` 的产物（它回写 npm 解析出的真实版本 + lockfile），单写者。
- **`createRequire` 的包含性判定必须 realpath 两边**：Node 解析器返回 realpath，macOS 临时目录是 `/var` → `/private/var`，不 realpath 会把真的在 app 里的包判成越界（和 `ui-compiler.ts` 的 `realDir()` 同一个坑）。
- **exceljs 在全局重声明 `interface Buffer extends ArrayBuffer`**：污染 `Buffer` 这个类型名。作者侧一律用 `Uint8Array`，交给 reader 一个 ArrayBuffer；`Buffer.from` 只当值用。
- **`scripts/check/templates.mts` 加 `types: ["node"]`**：后端模板合法使用 Node 全局（`Buffer`/`process`），host 的静态检查本来就白名单它们，门禁不该比运行时严。
- **`packages/ui/src/index.ts` 补 `export * from "./lib/illustrations"`**：skill 教 `Illu*`，但类型面看不到（运行时由 `scripts/build/ui.mjs` 注入 dist）。补上后组件数不变（仍 133），`check:skill` 绿。
- **模板清单的三处同步**：`smoke-test/fixture.ts`（+`NEEDS_PACKAGES`）、`templates-sweep`（注册后真装 exceljs）、`catalog-consistency`（8 个）、`scripts/dev/demo-templates.mts`（`NEEDS_PACKAGES` 装包，离线只 warn）。
- 顺带被类型门禁抓出一个真 bug：`parseWorkbook` 里多了一行没 `await` 的 `wb.xlsx.load(buffer)`（端到端照样跑通，纯靠门禁抓到）。

---

## 验收记录（2026-09-05）

- `pnpm verify` 12/12 绿（build / skill / examples / templates / lint / format / typecheck 12 配置 / test / dsh build）。
- `packages/smoke-test/templates-sweep.test.ts` 8/8，其中 `spreadsheet` 走真实 `mini_app_install`（真装 exceljs）后注册→编译 UI→`call("list")`。
- 真实 Chrome × 真实 demo host（`pnpm dev:host`，:17900）：拖入真 `.xlsx` → 侧栏历史、KPI、按表 Tabs、数值列「合计/均值/区间」、DataGrid 预览、`生成摘要` 走宿主模型（demo 假 caps）回填卡片并持久化「已摘要」。控制台只有 `favicon.ico` 404（与本任务无关）。
