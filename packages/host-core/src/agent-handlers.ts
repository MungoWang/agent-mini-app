import type { RuntimePort, CommitTree } from "./ports.js";
...
export type AgentHandlers = {
  ...
  mini_app_history_list: (input: {
    appId: string;
    limit?: number;
  }) => Promise<CommitTree | { commits: CommitTree }>;
...
export function createAgentHandlers(ctx: AgentCoreContext): AgentHandlers {
  ...
  return {
    ...
    async mini_app_history_list({ appId, limit }): Promise<CommitTree | { commits: CommitTree }> {
      const tree = await runtime.historyList(appId, { limit });
      if (tree && typeof tree === "object" && !Array.isArray(tree)) return tree as CommitTree;
      return { commits: tree as CommitTree };
    },
