...
1. UI imports `@monkey-mini-app/ui` (+ `react`) and the platform vendors `lodash` / `lodash-es` and `motion` / `motion/react`; backend imports `@monkey-mini-app/api` (`defineApp`) and the same `lodash` — **never `motion`** (no React there). UI may not import `main.api.ts`, `api/**`, or any other npm package. Hooks from `react`; components, `useApp()`, and `cn` from the UI package. Compiler: `react` → `/mma/runtime.js`, UI kit → `/mma/sdk.js`, `vendors/<lib>.js` per library (`lodash` → `/mma/vendors/lodash.js`, `motion` → `/mma/vendors/motion.js`, react external → the runtime); relative imports that leave the app dir fail. The vendor list is one table — `packages/host/src/compile/platform-modules.ts` — read by the UI compiler, the backend loader, the gateway route and `scripts/build/sdk.mjs`; do not add a specifier to just one of them. Legacy `@monkey-mini-app/sdk` / `@monkeyagent/*` are **removed**.
