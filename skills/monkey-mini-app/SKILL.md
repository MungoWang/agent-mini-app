...
- **Tailwind is compiled per app from your source** — variants (`hover:` `group-hover:` `md:`), the default palette (`bg-rose-500`) and arbitrary values (`w-[437px]`) work **when the installed host has the runtime Tailwind CLI** (shipped with the plugin from 0.1.11). A 500 on `/api/app/<id>/ui.css` means the app sheet never built — kit classes still work via `/ui.css`, which looks "styled" and is wrong. Class names must be complete literals (`` `bg-${x}-500` `` generates nothing). Token opacity (`bg-card/60`) and `backdrop-blur-*` often fail against the host skin — use `color-mix(in oklch, var(--card) 60%, transparent)`. The UI build **strips types but does not typecheck** (wrong `useStore` keys → runtime `TypeError`) → **[references/styling.md](references/styling.md)**
...
- [ ] `mini_app_view_eval` confirms the styling actually applied (no hardcoded hex, no composed class names); `/api/app/<id>/ui.css` is 200 and `.autogen/` exists
