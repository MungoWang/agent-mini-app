...
export default function Ui() {
  ...
  // ⭐ key: DataGrid columns declare sorting/column-search via meta; StatusBadge expects lowercase statuses
    ...
    <AppShell header={<PageHeader title="修复基准" description="编辑 → diff → 用例表格，改完写回" />}>
      <div className="flex h-full min-h-0 flex-col gap-3 overflow-y-auto">
        ...
        <Card>
          ...
          <CardContent>
            {/* ⭐ key: DiffViewer takes original/modified (not the old oldText/newText) */}
        ...
        <Card>
          ...
          <CardContent>
            <LogViewer entries={logs.map((l) => ({ level: l.level, message: l.text }))} />
          </CardContent>
        </Card>
      </div>
    </AppShell>
