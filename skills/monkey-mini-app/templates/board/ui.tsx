...
export default function Ui() {
  ...
  const shown = statusFilter === "all" ? issues : issues.filter((i) => i.status === statusFilter);
    ...
    >
      <div className="flex h-full min-h-0 flex-col gap-3">
        {/* ⭐ Look: aurora-bento — a tessellation that fills the band, and every tile is a
            destination (filter / switch view), not a stat readout sitting there. See
            references/looks/aurora-bento.md. Tiles stay flush: no page gap around them. */}
        <div className="bg-muted grid grid-cols-2 gap-1.5 p-1.5 sm:grid-cols-6">
          <button
            type="button"
            onClick={() => setStatusFilter("all")}
            className={cn(
              "bg-foreground text-background col-span-2 rounded-lg p-4 text-left sm:row-span-2",
              statusFilter === "all" && "ring-2 ring-ring ring-offset-2 ring-offset-muted",
            )}
          >
            <p className="text-xs opacity-70">阶段看板</p>
            <p className="mt-2 text-2xl font-medium tabular-nums">{summary?.total ?? 0}</p>
          ...
          {STATUSES.map((s) => (
            <button
              ...
              className={cn("bg-card rounded-lg p-3 text-left", statusFilter === s && STATUS_TILE[s])}
            >
              <span className="flex items-center gap-1.5">
                <span className={cn("size-2 rounded-full", STATUS_DOT[s])} />
                <span className="text-muted-foreground text-xs">{s}</span>
              </span>
              <b className="mt-1 block text-lg tabular-nums">{byStatus[s] ?? 0}</b>
            </button>
          ))}
          <button
            type="button"
            onClick={() => setView(view === "kanban" ? "table" : "kanban")}
            className="bg-card col-span-2 flex items-center justify-between rounded-lg p-3 text-left text-sm sm:col-span-4"
          >
            <span className="flex items-center gap-2">
              {view === "kanban" ? <Icon.LayoutGrid size={14} strokeWidth={2} /> : <Icon.Table size={14} strokeWidth={2} />}
